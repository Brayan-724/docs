...
```typescript src/routes/_index.ts
export function GET() {
...
```typescript routes/user/_index.ts
import { type HTTPRequest } from "densky";
...
Okey, What is a raw request? The request wrapper contains a property named `raw` which its type is [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request). See more on [API Documentation](https://densky.apika.me/api/http/HTTPRequest).
